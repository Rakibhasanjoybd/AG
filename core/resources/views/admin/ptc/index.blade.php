                        ...
                        @forelse($ads as $ptc)
                            <tr>
                                ...
                                <td data-label="@lang('Plan')">
                                    @if(($ptc->plan_id ?? 0) > 0 && $ptc->plan)
                                        <span class="badge badge--info">{{ $ptc->plan->name }}</span>
                                    @elseif(($ptc->plan_id ?? 0) == 0)
                                        <span class="badge badge--secondary">@lang('All Plans')</span>
                                    @else
                                ...
                                <td data-label="@lang('Amount')" class="font-weight-bold">
                                    @if(($ptc->plan_id ?? 0) > 0 && $ptc->plan)
                                        {{ showAmount($ptc->plan->ptc_view_amount ?? 0) }} {{$general->cur_text}}
                                    @elseif(($ptc->plan_id ?? 0) == 0)
                                        <span class="text-muted">@lang('Plan-based')</span>
                                    @else
