...
@section('panel')
    <div class="row">
        <div class="col-12">
            ...
            <div class="d-flex flex-wrap gap-3 mt-4">
                ...
                <div class="flex-fill">
                    <button type="button" class="btn btn--info btn--shadow w-100 btn-lg" data-bs-toggle="modal" data-bs-target="#resetWalletPinModal">
                        <i class="las la-key"></i>@lang('Reset Wallet PIN')
                    </button>
                </div>
            </div>
    ...
    {{-- Reset Wallet PIN MODAL --}}
    <div id="resetWalletPinModal" class="modal fade" tabindex="-1" role="dialog">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title"><i class="las la-key me-2"></i>@lang('Reset Wallet PIN')</h5>
                    <button type="button" class="close" data-bs-dismiss="modal" aria-label="Close">
                        <i class="las la-times"></i>
                    </button>
                </div>
                <form action="{{ route('admin.users.reset.wallet.pin', $user->id) }}" method="POST">
                    @csrf
                    <div class="modal-body">
                        <div class="alert alert-warning">
                            <i class="las la-exclamation-triangle"></i>
                            @lang('This will reset the user\'s withdrawal PIN. The user will be notified about this change.')
                        </div>
                        <div class="form-group">
                            <label>@lang('New PIN') <small class="text-muted">(@lang('4-6 digits'))</small></label>
                            <input type="password" name="new_pin" class="form-control" placeholder="@lang('Enter new 4-6 digit PIN')" required minlength="4" maxlength="6" pattern="[0-9]{4,6}" inputmode="numeric">
                            <small class="text-muted">@lang('PIN must be 4-6 numeric digits only')</small>
                        </div>
                        <div class="form-group">
                            <label>@lang('Confirm PIN')</label>
                            <input type="password" id="confirm_pin" class="form-control" placeholder="@lang('Confirm new PIN')" required minlength="4" maxlength="6" pattern="[0-9]{4,6}" inputmode="numeric">
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn--dark" data-bs-dismiss="modal">@lang('Cancel')</button>
                        <button type="submit" class="btn btn--primary" id="resetPinBtn">@lang('Reset PIN')</button>
                    </div>
                </form>
            </div>
        </div>
...
<script>
    ...
    "use strict"
        ...
        // Reset Wallet PIN validation
        $('#resetWalletPinModal form').on('submit', function(e) {
            var newPin = $('input[name=new_pin]').val();
            var confirmPin = $('#confirm_pin').val();
            
            if (newPin !== confirmPin) {
                e.preventDefault();
                alert('@lang("PINs do not match. Please try again.")');
                return false;
            }
            
            if (!/^[0-9]{4,6}$/.test(newPin)) {
                e.preventDefault();
                alert('@lang("PIN must be 4-6 numeric digits.")');
                return false;
            }
        ...
        $('input[name=new_pin], #confirm_pin').on('input', function() {
