@if($seo)
    ...
    @php
        $faviconPath = getFilePath('logoIcon') . '/favicon.png';
        $faviconV = file_exists($faviconPath) ? ('?v=' . filemtime($faviconPath)) : '';
        $logoPath = getFilePath('logoIcon') . '/logo.png';
        $logoV = file_exists($logoPath) ? ('?v=' . filemtime($logoPath)) : '';
    @endphp
    <link rel="shortcut icon" href="{{ getImage($faviconPath, $faviconV) }}" type="image/x-icon">

    {{--<!-- Apple Stuff -->--}}
    <link rel="apple-touch-icon" href="{{ getImage($logoPath, $logoV) }}">
