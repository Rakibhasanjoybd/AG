...
class SiteController extends Controller
{
    ...
    {
        $pageTitle = 'Plans';
        // Use basic query - ordered() requires sort_order column from migration
        $plans = Plan::where('status',1)->orderBy('id', 'asc')->get();

        // Single responsive UI (mobile-first) for all devices
        return view($this->activeTemplate.'plans_v2', compact('pageTitle','plans'));
    }
    ...
    public function blogDetails($slug,$id){
        ...
        return view($this->activeTemplate.'blog.details',compact('blog','pageTitle'));
    }


    public function cookieAccept(){
        $general = gs();
        return back()->withCookie(cookie('gdpr_cookie', $general->site_name, 43200));
    }
    ...
    /**
     * Mark popup announcement as viewed by user
     */
    public function popupViewed(Request $request)
    {
        if (!auth()->check()) {
        ...
        $request->validate([
            'popup_id' => 'required|exists:popup_announcements,id'
        ]);

        $popup = \App\Models\PopupAnnouncement::find($request->popup_id);
        
        if ($popup) {
            $popup->markAsViewedBy(auth()->user());
